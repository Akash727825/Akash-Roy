...
from .models import Catagory,  Product, Customar, Order
...
def signup(request):
    if request.method == 'POST':
        ...
        if not error_massage:      
            cust= Customar(first_name=first_name, email=email, pass1=pass1, pass2=pass2)
...
def signinpage(request):
    if request.method == 'POST':
        ...
        customer= Customar.get_customer(email)
        customer.save()
        error_message = None
        if customer:
...
def checkout(request):
    if request.method == 'POST':
        ...
        for product in products:
            order = Order(customer = Customar(id=customer), product=product, price=product.price,
